...
- 「AI 分析」页面：列表勾选收藏的网页，以对话形式让 AI 讲解分析（优先浏览器内置本地模型，也可配置 OpenAI 兼容接口）
- 支持扩展界面与元数据本地化
...
- 不加载或执行远程代码
- 不向服务器传输保存的数据；「AI 分析」为例外：只有在用户主动配置了自定义接口时，所选网页的标题、链接、描述、笔记和标签才会发送到该接口（需用户授予对应域名权限）。使用浏览器内置模型时数据不出本机
- 当前声明权限：`activeTab`、`contextMenus`、`scripting`、`storage`；可选权限：`https://*/*`（仅用于用户配置的 AI 接口域名）
